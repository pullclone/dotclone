...
4. **Privilege escalation:** Main branch uses `doas` as primary and keeps `sudo` installed but disabled by default; a separate "with-sudo" branch enables both. Introduce a `my.security.access.sudoFallback.enable` option.
